...
function render() {
  ...
  listEl.innerHTML = alerts
    .map((a) => {
      ...
      let timeLeftHtml;
      if (a.active) {
        timeLeftHtml = fmtTime(remaining);
      } else if (a.pausedRemaining !== undefined && a.pausedRemaining !== null) {
        timeLeftHtml = `Paused at ${fmtTime(a.pausedRemaining)}`;
      } else {
        timeLeftHtml = "Paused";
      }
      return `
        <div class="brx-card ${a.active ? "" : "is-paused"}" data-id="${a.id}">
          ...
          <div class="brx-info">
            ...
            <div class="brx-timeleft" data-timeleft>${timeLeftHtml}</div>
          </div>
...
// Clean up interval when popup is closed (page unload)
